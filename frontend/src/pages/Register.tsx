...
export function Register() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      navigate('/verifica-tu-correo', { state: { email } });
    } catch (err: any) {
      const apiErrors = err?.response?.data?.errors;
      if (apiErrors?.length) {
        setError(apiErrors.map((e: any) => e.mensaje).join(' | '));
      } else {
  ...
  return (
    <AuthLayout
      ...
      subtitle="Solo toma un minuto. Ya tendrás acceso al panel."
    >
      <form className="auth-form" onSubmit={handleSubmit} noValidate>
        ...
        <label className="field">
          ...
          <div className="field-icon">
            <svg className="field-icon__icon" width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <path d="M4 20c1.4-4 4.4-6 8-6s6.6 2 8 6" />
            </svg>
            <input
              type="text"
              autoComplete="name"
        ...
        <label className="field">
          ...
          <small>Mínimo 8 caracteres, con una mayúscula y un número.</small>
        </label>

        <button type="submit" className="btn-primary" disabled={submitting}>
          {submitting ? <span className="spinner" /> : 'Registrarme'}
        </button>
